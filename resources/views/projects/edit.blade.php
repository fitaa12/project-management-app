<x-app-layout>
    <x-slot name="header">
        <h2 class="font-bold text-2xl text-gray-800 leading-tight">
            {{ __('Edit Proyek:') }} <span class="text-indigo-600">{{ $project->name }}</span>
        </h2>
        <p class="mt-2 text-md text-gray-600">Perbarui informasi proyek Anda di bawah ini.</p>
    </x-slot>
                ...
                <div class="p-6 lg:p-8 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('projects.update', $project) }}">
                        ...
                        <div class="mb-6"> {{-- Tambah margin bawah --}}
                            ...
                            <x-text-input id="name" class="block w-full p-3 border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 shadow-sm" type="text" name="name" :value="old('name', $project->name)" required autofocus placeholder="Masukkan nama proyek" /> {{-- Padding lebih besar, rounded-lg, placeholder --}}
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>

                        <div class="mb-6">
                            <x-input-label for="description" :value="__('Deskripsi')" class="mb-2" />
                            <textarea id="description" name="description" rows="5" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-lg shadow-sm block w-full p-3" placeholder="Jelaskan detail proyek">{{ old('description', $project->description) }}</textarea> {{-- Padding lebih besar, rounded-lg, placeholder --}}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6"> {{-- Gap lebih besar --}}
                            <div>
                                ...
                                <x-text-input id="start_date" class="block w-full p-3 border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 shadow-sm" type="date" name="start_date" :value="old('start_date', $project->start_date)" /> {{-- Padding lebih besar, rounded-lg --}}
                                <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="end_date" :value="__('Tanggal Selesai')" class="mb-2" />
                                <x-text-input id="end_date" class="block w-full p-3 border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 shadow-sm" type="date" name="end_date" :value="old('end_date', $project->end_date)" /> {{-- Padding lebih besar, rounded-lg --}}
